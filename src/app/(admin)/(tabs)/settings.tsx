  ...
  return (
    <View className="bg-white rounded-xl px-4 py-3 mb-2 border border-slate-100 flex-row items-center">
      ...
      <View className="flex-1 mr-2">
        ...
        {item.description ? (
          <Text className="text-xs text-slate-500" numberOfLines={1}>{item.description}</Text>
        ) : null}
        <Text className="text-xs text-slate-400 mt-0.5">Urutan: {item.order}</Text>
      </View>
      <View className="items-end gap-1">
        <View className={`rounded-full px-2 py-0.5 ${item.active ? 'bg-green-100' : 'bg-slate-100'}`}>
          <Text className={`text-xs font-medium ${item.active ? 'text-green-700' : 'text-slate-500'}`}>
...
export default function AdminSettingsScreen() {
  ...
  const [form, setForm] = useState<CategoryFormState>({
    name: '', description: '', icon: '', active: true, order: '0',
  });
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    adminRepository.getCategories(false)
      .then((data) => {
  ...
  const openCreate = () => {
    setEditingId(null);
    setForm({ name: '', description: '', icon: '', active: true, order: '0' });
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      <View className="bg-white px-5 pt-4 pb-3 border-b border-slate-100 flex-row items-center justify-between">
        <View>
          <Text className="text-lg font-bold text-slate-900">Manajemen Kategori</Text>
          <Text className="text-xs text-slate-500 mt-0.5">{categories.length} kategori</Text>
        </View>
        <TouchableOpacity
      ...
      {loading && !refreshing ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <CategoryRow item={item} onEdit={openEdit} />}
          ...
          ListEmptyComponent={
            <View className="items-center py-16">
              ...
              >
                <Text className="text-white font-semibold">Buat Kategori Pertama</Text>
              </TouchableOpacity>
            </View>
          }
        />
      ...
      <Modal visible={modalVisible} animationType="slide" presentationStyle="formSheet" onRequestClose={() => setModalVisible(false)}>
        <SafeAreaView className="flex-1 bg-slate-50">
          <View className="bg-white px-5 py-4 border-b border-slate-100 flex-row items-center justify-between">
            ...
            <TouchableOpacity onPress={() => setModalVisible(false)} id="admin-category-modal-close">
          ...
          <View className="px-5 pt-5 gap-4">
            <View>
              <Text className="text-xs font-semibold text-slate-600 mb-1.5">Nama Kategori *</Text>
              <TextInput
            ...
            <View>
              <Text className="text-xs font-semibold text-slate-600 mb-1.5">Deskripsi</Text>
              <TextInput
                className="bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-800"
            ...
            <View>
              <Text className="text-xs font-semibold text-slate-600 mb-1.5">Ikon (emoji)</Text>
              <TextInput
                className="bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-800"
            ...
            <View>
              <Text className="text-xs font-semibold text-slate-600 mb-1.5">Urutan Tampil</Text>
              <TextInput
                className="bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-800"
                placeholder="0"
                value={form.order}
            ...
            <TouchableOpacity
              className={`bg-blue-600 rounded-xl py-4 items-center mt-2 ${saving ? 'opacity-50' : ''}`}
              onPress={handleSave}
              disabled={saving}
