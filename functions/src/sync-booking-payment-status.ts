...
export interface SyncPaymentStatusPayload {
  bookingId: string;
}

export const syncBookingPaymentStatus = onCall(
  { secrets: [midtransServerKey] },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Pengguna harus terautentikasi.');
    }

    const uid = request.auth.uid;
    const userRole = request.auth.token?.app_role || 'customer';
    const data = request.data as SyncPaymentStatusPayload;
    const bookingId = (data.bookingId || '').trim();

    if (!bookingId) {
      throw new HttpsError('invalid-argument', 'bookingId wajib diisi.');
    }
    ...
    if (!bookingSnap.exists) {
      throw new HttpsError('not-found', 'Booking tidak ditemukan.');
    }
    ...
    if (userRole !== 'admin' && bookingData.customerId !== uid) {
      throw new HttpsError('permission-denied', 'Anda tidak memiliki akses ke pesanan ini.');
    }
    ...
    const targetPaymentStatus = mapMidtransStatus(transactionStatus, fraudStatus);
