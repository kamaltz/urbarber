  ...
  async (request) => {
    ...
    if (!request.auth) {
      throw new HttpsError(
        'unauthenticated',
        'Pengguna harus terautentikasi untuk membuat pesanan.'
      );
    }

    const customerId = request.auth.uid;
    const data = request.data as CreatePaymentPayload;
    ...
    if (!barberId || !serviceId || !date || !startTime || !address) {
      throw new HttpsError(
        'invalid-argument',
        'Parameter booking tidak lengkap (barberId, serviceId, date, startTime, address wajib diisi).'
      );
    }
    ...
    if (userStatus === 'suspended') {
      throw new HttpsError('permission-denied', 'Akun pelanggan sedang ditangguhkan.');
    }
    ...
    if (!barberSnap.exists) {
      throw new HttpsError('not-found', 'Barber tidak ditemukan.');
    }
    const barberData = barberSnap.data();
    if (barberData?.status !== 'active' || barberData?.verified !== true) {
      throw new HttpsError('failed-precondition', 'Barber tidak aktif atau belum terverifikasi.');
    }

    // 6. Verify Service
    if (!serviceSnap.exists) {
      throw new HttpsError('not-found', 'Layanan barber tidak ditemukan.');
    }
    const serviceData = serviceSnap.data();
    if (serviceData?.barberId !== barberId || serviceData?.active === false) {
      throw new HttpsError('failed-precondition', 'Layanan tidak valid atau tidak aktif untuk barber ini.');
    }
    ...
    if (grossAmount <= 0) {
      throw new HttpsError('invalid-argument', 'Harga layanan tidak valid.');
    }
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        if (slotSnap.exists) {
          ...
          if (slotData?.status === 'locked' || slotData?.status === 'booked') {
            throw new HttpsError('already-exists', 'Jadwal slot waktu yang dipilih sudah dipesan.');
          }
        }
      ...
      throw new HttpsError(
        'aborted',
        err?.message || 'Gagal mengunci jadwal booking. Silakan coba waktu lain.'
      );
    }
    ...
    try {
      ...
      throw new HttpsError(
        'internal',
        'Gagal menghubungkan ke layanan pembayaran Midtrans. Silakan coba lagi.'
      );
    }
  }
);
